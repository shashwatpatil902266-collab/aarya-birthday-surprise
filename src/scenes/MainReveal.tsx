  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        <CanvasErrorBoundary fallbackGradient="bg-pink-100">
          <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0, 5], fov: 50 }}>
            <ambientLight intensity={0.6} />
            <directionalLight position={[10, 10, 5]} intensity={1} color="#fbcfe8" />
            {!prefersReducedMotion && <Sparkles count={36} scale={12} size={3} speed={0.2} opacity={0.5} color="#ec4899" />}
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-4 sm:p-6 text-center pointer-events-none">
        <motion.div
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ duration: 1.5, type: "spring" }}
          className="relative bg-white/40 backdrop-blur-md p-6 sm:p-10 rounded-3xl border border-white/60 shadow-xl max-w-2xl pointer-events-auto"
        >
